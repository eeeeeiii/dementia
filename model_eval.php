          ...
          <?php
            ...
            if ($_FILES['csv']['error'] === UPLOAD_ERR_OK){  
              ...
              if (in_array($_FILES['csv']['type'], $allowedMimeTypes)) {
                ...
                $_FILES['csv']['name']="upload_data.csv";
              ...
              <div class="row">
                <div class="col-xl-12">
                  <div class="card" style="max-height:max-content">
                    <div class="card-body">
                      <h3 class="fw-bold py-3 mb-4" style='text-align:center'><span class="text">模型再評估</span></h3>
                          ...
                          <script>
                            ...
                            $(document).ready(function() {
                              $("#evaluate").on("click", function() {
                                axios.post('http://127.0.0.1:8000/evaluate')
                                .then(response => {
                                   //找max
                                   var max_Avg_acc = Math.max(
                                    parseFloat(response.data.Avg_cart_acc),
                                    parseFloat(response.data.Avg_log_acc),
                                    parseFloat(response.data.Avg_old_acc)
                                  );

                                  var max_testing_acc=Math.max(
                                    parseFloat(response.data.cart_acc),
                                    parseFloat(response.data.log_acc),
                                    parseFloat(response.data.old_acc)
                                  );

                                  var max_testing_f1s=Math.max(
                                    parseFloat(response.data.cart_f1s),
                                    parseFloat(response.data.log_f1s),
                                    parseFloat(response.data.old_f1s)
                                  );   
                                  var max_testing_pre=Math.max(
                                    parseFloat(response.data.cart_pre),
                                    parseFloat(response.data.log_pre),
                                    parseFloat(response.data.old_pre)
                                  );

                                  var max_testing_sen=Math.max(
                                    parseFloat(response.data.cart_sen),
                                    parseFloat(response.data.log_sen),
                                    parseFloat(response.data.old_sen)
                                  );

                                  var max_testing_spe=Math.max(
                                    parseFloat(response.data.cart_spe),
                                    parseFloat(response.data.log_spe),
                                    parseFloat(response.data.old_spe)
                                  );
                
                                  //python回傳結果
                                  function updateValueAndCheckMax(value, maxValue, element) {
                                    element.innerHTML = value;
                                    if (value == maxValue) {
                                      element.classList.add("display_max_value");
                                    }
                                  }
                                  ...
                                  //CART
                                  updateValue(response.data.Avg_cart_acc, document.getElementById('trainning_acc_cart'));
                                  updateValueAndCheckMax(response.data.cart_acc, max_testing_acc, document.getElementById('testing_acc_cart'));
                                  updateValueAndCheckMax(response.data.cart_f1s, max_testing_f1s, document.getElementById('testing_f1s_cart'));
                                  updateValueAndCheckMax(response.data.cart_pre, max_testing_pre, document.getElementById('testing_pre_cart'));
                                  updateValueAndCheckMax(response.data.cart_sen, max_testing_sen, document.getElementById('testing_sen_cart'));
                                  updateValueAndCheckMax(response.data.cart_spe, max_testing_spe, document.getElementById('testing_spe_cart'));
                                  //LOG
                                  updateValue(response.data.Avg_log_acc, document.getElementById('trainning_acc_log'));
                                  updateValueAndCheckMax(response.data.log_acc, max_testing_acc, document.getElementById('testing_acc_log'));
                                  updateValueAndCheckMax(response.data.log_f1s, max_testing_f1s, document.getElementById('testing_f1s_log'));
                                  updateValueAndCheckMax(response.data.log_pre, max_testing_pre, document.getElementById('testing_pre_log'));
                                  updateValueAndCheckMax(response.data.log_sen, max_testing_sen, document.getElementById('testing_sen_log'));
                                  updateValueAndCheckMax(response.data.log_spe, max_testing_spe, document.getElementById('testing_spe_log'));
                                  //OLD
                                  updateValue(response.data.Avg_old_acc, document.getElementById('trainning_acc_old'));
                                  updateValueAndCheckMax(response.data.old_acc, max_testing_acc, document.getElementById('testing_acc_old'));
                                  updateValueAndCheckMax(response.data.old_f1s, max_testing_f1s, document.getElementById('testing_f1s_old'));
                                  updateValueAndCheckMax(response.data.old_pre, max_testing_pre, document.getElementById('testing_pre_old'));
                                  updateValueAndCheckMax(response.data.old_sen, max_testing_sen, document.getElementById('testing_sen_old'));
                                  updateValueAndCheckMax(response.data.old_spe, max_testing_spe, document.getElementById('testing_spe_old'));

                                  
                                  const data_nums  = document.getElementById("data_nums");
                                  data_nums.value =  response.data.de_up_nums;

                                  if(response.data.cart_acc > response.data.old_acc ){
                                    swal({
                                      title:"建議更改模型",
                                      icon: "info",
                                      text:"(CART ACC優於原本模型)"
                                  ...
                                  if(response.data.log_acc > response.data.old_acc ){
                                    swal({
                                      title:"建議更改模型",
                                      icon: "info",
                                      text:"(Logistic ACC優於原本模型)",
                                      buttons:{
                                      確定更改: {
                                          text: "確定更改",
                                          value: "確定更改"
                                    ...
                                    .then((value) => {
                                      switch (value) {
                                        case "確定更改":
                                          ...
                                          .then((value,) => {
                                            axios.post('http://127.0.0.1:8000/rebuild')
                                            .then(response => {
                                                swal({
                                                title: "更改完成",
                                                icon: "success",
                        ...
                        <div id="output" style="display:none;margin-top:5%">
                          <table>
                            <tr>
                              ...
                              <th style="border-left:solid 2px #E0E0E0;border-bottom:solid 2px #E0E0E0;padding-left:1%">Trainning<br> ACC</th>
                              <th style="border-bottom:solid 2px #E0E0E0;">Testing <br>ACC</th>
                            ...
                            <tr>
                              <th style="border-right:solid 2px #E0E0E0;">Logistic</th>
                              <td id="trainning_acc_log"></td>
                              <td id="testing_acc_log"></td>
                              <td id="testing_f1s_log"></td>
                              <td id="testing_pre_log"></td>
                              <td id="testing_sen_log"></td>
                              <td id="testing_spe_log"></td>
                            </tr>
                            <tr>
                              <th style="border-right:solid 2px #E0E0E0;">原本模型</th>
                              <td id="trainning_acc_old"></td>
                              <td id="testing_acc_old"></td>
                              <td id="testing_f1s_old"></td>
                              <td id="testing_pre_old"></td>
                              <td id="testing_sen_old"></td>
                              <td id="testing_spe_old"></td>
                            </tr>
                          </table>
                          <br><br>
                          <table>
